...
def estimation_and_performance_check_in_regression_train_and_test(model, x_train, y_train, x_test, y_test):
    ...
    # plt.rcParams['font.size'] = 18  # 横軸や縦軸の名前の文字などのフォントのサイズ
    # plt.figure(figsize=figure.figaspect(1))  # 図の形を正方形に
    # plt.scatter(y_test, estimated_y_test.iloc[:, 0], c='red')  # 実測値 vs. 推定値プロット
    ...
    # 学習データ、テストデータをまとめてプロット
    plt.figure(figsize=(6,6))
    
    plt.scatter(y_train, estimated_y_train.iloc[:, 0], c='blue', label='train')
    plt.scatter(y_test, estimated_y_test.iloc[:, 0], c='red', label='test')
    y_all = pd.concat([y_train, y_test])
    y_est_all = pd.concat([estimated_y_train.iloc[:, 0], estimated_y_test.iloc[:, 0]])
    y_max = max(y_all.max(), y_est_all.max())
    y_min = min(y_all.min(), y_est_all.min())
...
def estimation_and_performance_check_in_classification_train_and_test(model, x_train, y_train, x_test, y_test):
    ...
    plt.figure()
    sns.heatmap(confusion_matrix_test, annot=True, cmap='Reds', fmt='d')
